...
- **Routing** — the fee is only taken on chains where the send path actually routes it, so the disclosure can never claim a fee the wallet does not collect:
  ...
  - **Configurable, routing pending:** ETH, TRX, USDT-TRC20 — deferred because a `%` fee needs a second transaction that is uneconomic on ETH gas / TRON energy (a same-transaction fee there needs a batching contract). Web has no on-chain send at all, so on web the percentage only drives the disclosed Exchange quote.
